...
\begin{document}
  ...
  \begin{abstract}
    ...
    $V_{\mathrm{color}}$, conditional on hypothesis~[H-color].
  \end{abstract}
  ...
  \subsection{Conditional status and dependencies}
    ...
    \end{equation}
    where $V_{\mathrm{color}} \in \mathrm{Rep}(SU(3))$.
    ...
    The colour-coupled quark bundle $S_\Pi \otimes V_{\mathrm{color}}$ is conditional
    on hypothesis~[H-color], as inherited from~\cite{Beau2026a35,Beau2026a36}.
  ...
  \subsection{Organisation}
    ...
    Section~\ref{sec:colour} discusses the colour-coupled quark sector and its
    conditional status under~[H-color].
  ...
  \subsection{Hypercharge rigidity}
    ...
    \begin{proposition}[Hypercharge rigidity from spectral coherence]
      ...
      matter decomposition~\eqref{eq:matter-content}, with the colour factor conditional
      on~\emph{[H-color]}.
  ...
  \subsection{Status of the colour factor}
    ...
    By contrast, the factor $V_{\mathrm{color}}$ inherits the conditional status of
    the $SU(3)$ sector.

    The identification of $SU(3)$ as the colour gauge group depends on the colour
    co-admissibility hypothesis~[H-color], according to which the admissible colour
    triplet sectors are co-admissible in the pre-saturation window.
    This hypothesis is supported numerically for $q \in \{61, 151, 211\}$ by the
    O32 campaign~\cite{Beau2026a36}, but its analytical derivation from the Foundation
    axioms remains open.
    Under~[H-color], the triplet colour module defines
    $V_{\mathrm{color}} \in \operatorname{Rep}(SU(3))$ and the quark
    sector~\eqref{eq:quark-bundle} is well defined.
    Without~[H-color], the electroweak spinorial sector remains intact, but the
    colour-coupled quark sector is not claimed as an unconditional output of the
    present paper.

    \begin{proposition}[Conditional colour extension]
      \label{prop:conditional-colour-extension}
      Assume~\emph{[H-color]}, so that the admissible colour triplet defines a
    ...
    \begin{remark}
      ...
      bundle $S_\Pi$.
      It adds an independent gauge representation factor, conditional on the
      colour-sector result.
      The logical status of the fermionic construction is therefore stratified:
      \begin{equation}
        \label{eq:status-table}
        \begin{cases}
          S_\Pi \otimes \mathbb{C}^3_{\mathrm{gen}}
          & \text{electroweak/leptonic carrier, independent of [H-color],} \\[4pt]
          S_\Pi \otimes V_{\mathrm{color}} \otimes \mathbb{C}^3_{\mathrm{gen}}
          & \text{quark carrier, conditional on [H-color].}
        \end{cases}
      \end{equation}
    \end{remark}
  ...
  \]
  This step inherits the conditional status of the colour co-admissibility
  hypothesis~[H-color].
  ...
  \[
    ...
    \mathbb{C}^3_{\mathrm{gen}},
  \]
  with the first summand independent of~[H-color] and the second conditional on it.
  ...
  Third, the analytical derivation of~[H-color] from the Foundation axioms remains
  the open condition for promoting the quark colour sector from conditional to
  ...
  Its spinorial, electroweak, chiral, and generation structures arise from distinct
  functorial readings of the same admissible Weil fibre, while the colour sector
  retains the explicit conditional status inherited from the gauge-fibre programme.
  The remaining open problems are not structural gaps in the architecture, but
  specific rigidity problems for $E_\Pi$, $L_Y$, and~[H-color].
